...
public class DirectMessagingProvider : IDirectMessagingService
{
    ...
    public Result<DirectMessaging> Get(long userId, long directMessagingId) =>
        _burstChatContext
            .Map(bc =>
                bc.DirectMessaging.FirstOrDefault(dm =>
                    dm.Id == directMessagingId
                    && (dm.FirstParticipantUserId == userId || dm.SecondParticipantUserId == userId)
                )
            )
            .And(dm => dm?.Ok() ?? DirectMessagingErrors.DirectMessageNotFound)
            .InspectErr(e => _logger.LogError(e.Message));

    public Result<DirectMessaging> Get(
        long userId,
        ...
        (
            ...
            .Map(_ =>
                _burstChatContext
                    .DirectMessaging.Include(dm => dm.FirstParticipantUser)
                    .Include(dm => dm.SecondParticipantUser)
                    .FirstOrDefault(dm =>
                        (
                            dm.FirstParticipantUserId == firstParticipantId
                            && dm.SecondParticipantUserId == secondParticipantId
                        )
                        || (
                            dm.FirstParticipantUserId == secondParticipantId
                            && dm.SecondParticipantUserId == firstParticipantId
                        )
                    )
            )
            .And(dm => dm?.Ok() ?? DirectMessagingErrors.DirectMessageNotFound)
    ...
    public Result<IEnumerable<User?>> GetUsers(long userId) =>
        _burstChatContext
            .Map(bc =>
                bc.DirectMessaging.Include(dm => dm.FirstParticipantUser)
                    .Include(dm => dm.SecondParticipantUser)
                    .Where(dm =>
                        dm.FirstParticipantUserId == userId || dm.SecondParticipantUserId == userId
                    )
    ...
    public Result<DirectMessaging> Insert(
        long userId,
        long firstParticipantId,
        long secondParticipantId
    ) =>
        _burstChatContext
            .And(bc =>
            {
                var isProperUser = firstParticipantId == userId || secondParticipantId == userId;

                var directMessaging = bc.DirectMessaging.FirstOrDefault(dm =>
                    (
                        dm.FirstParticipantUserId == firstParticipantId
                        && dm.SecondParticipantUserId == secondParticipantId
                    )
    ...
    public Result<DirectMessaging> Delete(long userId, long directMessagingId) =>
    ...
    public Result<IEnumerable<Message>> GetMessages(
        ...
        Get(userId, directMessagingId)
            .Map(_ =>
                _burstChatContext
                    .DirectMessaging.Include(dm => dm.Messages)
                    ...
                    .Select(dm =>
                        dm.Messages.Where(m =>
                            m.Id < (lastMessageId ?? long.MaxValue)
    ...
    public Result<Message> InsertMessage(long userId, long directMessagingId, Message message) =>
        Get(userId, directMessagingId)
            .And(dm =>
            {
    ...
    public Result<Message> DeleteMessage(long userId, long directMessagingId, long messageId) =>
        Get(userId, directMessagingId)
            .And(_ =>
            {
                var directMessaging = _burstChatContext
                    .DirectMessaging.Include(dm => dm.Messages.Where(m => m.Id == messageId))
