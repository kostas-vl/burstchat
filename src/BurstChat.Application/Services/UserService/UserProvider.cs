...
public class UserProvider : IUserService
{
    ...
    private Result<Unit> AlphaInvitationCodeExists(Guid alphaInvitationCode) => _burstChatContext
        .Map(bc => bc.AlphaInvitations.Any(a => a.Code == alphaInvitationCode && a.DateExpired >= DateTime.UtcNow))
        .And(codeExists => codeExists ? Unit.Ok : AlphaInvitationErrors.AlphaInvitationCodeIsNotValid)
        .InspectErr(e => _logger.LogError(e.Message));


    public Result<User> Get(long id) => _logger
    ...
    public Result<User> Get(string email) => _burstChatContext
        .Map(bc => bc.Users.FirstOrDefault(u => u.Email == email))
        .And(u => u?.Ok() ?? UserErrors.UserNotFound)
        .InspectErr(e => _logger.LogError(e.Message));


    public Result<User> Insert(Guid alphaInvitationCode, string email, string name, string password) => Get(email)
    ...
    public Result<User> Update(User user) => (user?.Ok() ?? UserErrors.UserNotFound)
    ...
    public Result<Unit> Delete(long id) => Get(id)
        .Map(user =>
        {
    ...
    public Result<IEnumerable<Server>> GetSubscriptions(long userId) => _burstChatContext
        .Map(bc => bc
            .Servers
    ...
    public Result<IEnumerable<PrivateGroup>> GetPrivateGroups(long userId) => Get(userId)
        .Map(user => _burstChatContext
            .PrivateGroups
            .Include(pmg => pmg
                .Users
    ...
    public Result<IEnumerable<DirectMessaging>> GetDirectMessaging(long userId) => Get(userId)
        .Map(user => _burstChatContext
            .DirectMessaging
            .Where(d =>
                d.FirstParticipantUserId == user.Id
    ...
    public Result<User> Validate(string email, string password) => Get(email)
        .And(user => _bcryptService.VerifyHash(password, user.Password)
            ? user.Ok()
    ...
    public Result<string> IssueOneTimePassword(string email) => _burstChatContext
        .And(bc =>
        {
    ...
    public Result<Unit> ChangePassword(string email, string oneTimePass, string password) => _burstChatContext
        .Map(bc => bc.Users.FirstOrDefault(u => u.Email == email))
    ...
    public Result<IEnumerable<Invitation>> GetInvitations(long userId) => _burstChatContext
        .Map(bc => bc
            .Invitations
            .Include(i => i.Server)
            .Include(i => i.User)
    ...
    public Result<Invitation> UpdateInvitation(long userId, UpdateInvitation data) => _burstChatContext
        .Map(bc => bc
            .Invitations
            .Include(i => i.Server)
            .Include(i => i.User)
    ...
    public Result<IEnumerable<Claim>> GetClaims(User user) => (user?.Ok() ?? UserErrors.UserNotFound)
        .Map(user => new Claim[]
        {
