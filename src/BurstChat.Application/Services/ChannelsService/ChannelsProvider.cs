...
public class ChannelsProvider : IChannelsService
{
    ...
    private Result<Server> GetServer(long userId, int channelId) =>
        _burstChatContext
            .Map(bc =>
    ...
    public Result<Channel> Get(long userId, int channelId) =>
        Get(userId, channelId)
            .Map(_ => _burstChatContext.Channels.FirstOrDefault(c => c.Id == channelId))
            .And(channel =>
                channel is Channel { IsPublic: true } ? channel.Ok() : ChannelErrors.ChannelNotFound
            )
            .InspectErr(e => _logger.LogError(e.Message));

    public Result<Channel> Insert(long userId, int serverId, Channel channel) =>
        _serversService
            .Get(userId, serverId)
            .Map(server =>
            {
    ...
    public Result<Channel> Update(long userId, Channel channel) =>
        channel
    ...
    public Result<Channel> Delete(long userId, int channelId) =>
        Get(userId, channelId)
            .Map(channel =>
            {
    ...
    public Result<Message> InsertMessage(long userId, int channelId, Message message) =>
        Get(userId, channelId)
            .And(channel =>
            {
    ...
    public Result<Message> UpdateMessage(long userId, int channelId, Message message) =>
        message
            ...
            .And(m =>
            {
                ...
                if (entries.Count != 1)
                    return ChannelErrors.ChannelMessageNotFound;
    ...
    public Result<Message> DeleteMessage(long userId, int channelId, long messageId) =>
        _burstChatContext
            .Map(bc =>
                bc.Channels.Include(c => c.Messages.Where(m => m.Id == messageId))
                    .First(c => c.Id == channelId)
            )
            .And(channel =>
                channel.Messages.Any() ? channel.Ok() : ChannelErrors.ChannelMessageNotFound
            )
            .Map(channel =>
            {
