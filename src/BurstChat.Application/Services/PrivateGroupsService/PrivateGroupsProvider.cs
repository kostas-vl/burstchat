...
public class PrivateGroupsProvider : IPrivateGroupsService
{
    ...
    public Result<PrivateGroup> Get(long userId, long groupId) => _burstChatContext
        .And(bc =>
        {
            var privateGroup = bc
                .PrivateGroups
    ...
    public Result<PrivateGroup> Insert(long userId, string groupName) => _burstChatContext
        .And(bc =>
        {
    ...
    public Result<Unit> Delete(long userId, long groupId) => Get(userId, groupId)
        .Map(privateGroup =>
        {
    ...
    public Result<PrivateGroup> InsertUser(long userId, long groupId, long newUserId) => Get(userId, groupId)
        .And(privateGroup =>
    ...
    public Result<PrivateGroup> InsertUsers(long userId, long groupId, IEnumerable<long> userIds) => Get(userId, groupId)
        .Map(privateGroup =>
        {
    ...
    public Result<PrivateGroup> DeleteUser(long userId, long groupId, long targetUserId) => Get(userId, groupId)
        .And(privateGroup =>
    ...
    public Result<IEnumerable<Message>> GetMessages(long userId, long groupId) => _burstChatContext
        .Map(bc => bc.PrivateGroups.Include(pgm => pgm.Messages).FirstOrDefault(pgm => pgm.Id == groupId))
        .And(pg => pg?.Ok() ?? PrivateGroupErrors.GroupNotFound)
        .And(pg =>
        {
            var userInList = pg.Users.Any(u => u.Id == userId);
            return userInList ? pg.Messages.AsEnumerable().Ok() : PrivateGroupErrors.GroupNotFound;
        })
        .InspectErr(e => _logger.LogError(e.Message));


    public Result<Message> InsertMessage(long userId, long groupId, Message message) => Get(userId, groupId)
        .And(grp =>
        {
            var user = _burstChatContext.Users.FirstOrDefault(u => u.Id == userId);
    ...
    public Result<Message> UpdateMessage(long userId, long groupId, Message message) => GetMessages(userId, groupId)
        .Map(gm => gm.FirstOrDefault(m => m.Id == message.Id))
        .And(sm => sm is not null && message is not null ? sm.Ok() : PrivateGroupErrors.GroupMessageNotFound)
        .Map(sm =>
        {
    ...
    public Result<Unit> DeleteMessage(long userId, long groupId, long messageId) => GetMessages(userId, groupId)
