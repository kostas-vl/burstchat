            ...
            Host.CreateDefaultBuilder(args)
                .ConfigureWebHostDefaults(webBuilder =>
                {
                    webBuilder
                        ...
                        {
                            config.AddJsonFile(
                                "appsettings.AcceptedDomains.json",
                                optional: false,
                        ...
                        .UseKestrel(options =>
                        {
                            var envHost = Environment.GetEnvironmentVariable(
                                EnvironmentVariables.BURST_CHAT_SIGNAL_HOST
                            );
                            var envPort = Environment.GetEnvironmentVariable(
                                EnvironmentVariables.BURST_CHAT_SIGNAL_PORT
                            );
                            ...
                            if (envHost != null && envPort != null)
                            {
                                ...
                                if (!canParseHost)
                                    throw new Exception(
                                        $"{EnvironmentVariables.BURST_CHAT_SIGNAL_HOST} invalid value"
                                    );

                                var canParsePort = Int32.TryParse(envPort, out var port);
                                if (!canParsePort)
                                    throw new Exception(
                                        $"{EnvironmentVariables.BURST_CHAT_SIGNAL_PORT} invalid value"
                                    );
